...
import { ProductSingleType } from '../../types/pagesTypes'
...
const ProductSinglePage = () => {
	...
	useEffect(() => {
		if (id !== undefined) {
			dispatch(fetchAsyncProductSingle(Number(id)))
		}
	...
	const addToCartHandler = (product: ProductSingleType) => {
		let discountedPrice =
	...
	const checkoutHandler = (product: ProductSingleType) => {
		let discountedPrice =
			product?.price - (product?.price * (product?.discountPercentage || 0)) / 100
		let totalPrice = quantity * discountedPrice
		dispatch(addToCart({ ...product, quantity: quantity, totalPrice, discountedPrice }))
							...
							<div className="product-img">
								<div className="product-img-zoom">
									<img
										...
										alt=""
										className="img-cover product-main-img"
									/>
								</div>
