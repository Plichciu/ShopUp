...
import { productType } from '../../types/pagesTypes'
...
const HomePage = () => {
	...
	if (products.length > 0) {
		for (let i in products) {
			...
			tempProducts[Number(i)] = products[randomIndex]
		}
	}

	let catProductsOne = products.filter(
		(product: productType) => product.category === categories[0]
	)
	let catProductsTwo = products.filter(
		(product: productType) => product.category === categories[1]
	)
	let catProductsThree = products.filter(
		(product: productType) => product.category === categories[2]
	)
	let catProductsFour = products.filter(
		(product: productType) => product.category === categories[3]
	)
